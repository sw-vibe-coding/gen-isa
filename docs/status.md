...
2026-05-10 — saga step 11 (`ibm1130-emulator`) complete. `sw-comp-history/sw-ibm1130-emulator` pushed: minimal CPU + word-addressed memory + step/run executor, 13 instruction-semantics unit tests, and 4 curated demo programs (math, conditions, loops, strings) that assemble through `-asm` and run to halt with verified register/memory state. BSC condition-mask semantics pinned (postmortem note: long-form mask bits live in the spec's reserved area, currently dropped). Character-encoding plan deferred to its own future saga; design captured in `docs/character-encoding-plan.md`.
...
- 2026-05-09: ABI revision (mid-step-9 course-correction). Research against bitsavers listings (IBM 1130 Subroutine Library C26-5929-4 and FORTRAN compiler conventions) showed step 8's invented ABI conflicted with the historical 1130 idiom: XR3 is the LIBF transfer-vector base (loader-managed program-lifetime invariant), not a frame pointer. New `docs/abi-linkage.md` captures the research. Updated [sw-ibm1130-target](https://github.com/sw-comp-history/sw-ibm1130-target) `docs/abi.md` and impls: XR3 moved to `RegisterClasses::reserved()` and forbidden in every CallingConvention slot; XR2 promoted to single frame-base/SP register (no separate FP, fixed-size frames per historical practice); caller-saved = {ACC, EXT, XR1}, callee-saved = {XR2}. 15 smoke tests including defensive XR3-reserved checks. Step 9 codegen now resumes against the corrected ABI.
- 2026-05-09: completed step `ibm1130-codegen`. [`sw-comp-history/sw-ibm1130-codegen`](https://github.com/sw-comp-history/sw-ibm1130-codegen). Implements `sw_codegen_core::Backend` for the IBM 1130 with hand-written per-TIR-op lowering: arith (Add/Sub/Mul/SDiv via LD;OP;STO), mem (Load/Store via LD I / STO I), cmp_branch (Icmp via S+BSC mask, Branch+CondBranch via BSC long with fixups), call (Call via BSI long+inline DCs, Return via BSC I entry-word). Naive slot-based register model: ValueId(N) bound to memory word slot N; ACC is the single computational register. Calling convention follows `sw-ibm1130-target/docs/abi.md` (ACC for first arg + scalar return; inline DC words for additional args; XR3 never touched). 8 snapshot-style tests plus 1 smoke test pass; cargo build/test/clippy -D warnings/fmt --check all clean. Deferred to later steps: exact BSC mask semantics (step 11 emulator), indirect calls, i32-result EXT spilling, real DC emission for inline args (step 10 asm), branch relaxation (step 11 emulator).
- 2026-05-09: completed step `ibm1130-asm`. [`sw-comp-history/sw-ibm1130-asm`](https://github.com/sw-comp-history/sw-ibm1130-asm). Hand-rolled lexer + recursive-descent parser; two-pass assembly (build symtab, then emit). All 24 mnemonics (ld/ldd/sto/std/ldx/stx/lds/sts/a/ad/s/sd/m/d/and/or/eor/sla/sra/bsc/bsi/mdx/wait/xio) + ORG/EQU/DC/END directives. Source format: `LABEL: MNEMONIC [I] [L] [TAG,] EXPR [, MASK] ; comment`. Encoding via `sw_ibm1130_isa::encode`; disassembly via `sw_ibm1130_isa::decode`. Round-trip contract: text -> bytes -> text -> bytes' produces bytes == bytes' for all-instruction sources. 27 integration tests + 1 smoke test cover per-mnemonic assembly, short/long form selection, TAG/indirect encoding, forward label references, all 24-mnemonic round-trip, and 6 negative tests (unknown mnemonic, undefined symbol, out-of-range short displacement, duplicate label, ORG-with-label, wait-with-L-flag) with line/column position reporting. cargo build/test/clippy -D warnings/fmt --check all clean. Deferred: literal expressions (`SYM+1`), proper handling of codegen's pseudo-DC inline arg blocks, round-trip through ORG-padded zero regions.
